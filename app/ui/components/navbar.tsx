...
export const Navbar = () => {
  ...
  return (
    <nav className="flex justify-between items-center pt-6 md:pt-10 sticky top-0 z-50">
      <Link
        href={"/"}
      ...
      <ul className={`${!isNavOpened? "-translate-x-0": "-translate-x-full"} transition duration-200 flex flex-col md:flex-row items-center gap-14 md:gap-10 lg:gap-[60px] absolute md:relative top-0 left-0 pt-28 md:pt-0 pr-6 md:pr-10 lg:pr-20 w-full md:w-auto h-screen md:h-fit bg-black md:bg-transparent bg-opacity-80 md:bg-opacity-100 backdrop-blur-md md:backdrop-blur-0`}>
        {navLinks.map((link, idx) => (
          <li key={idx} onClick={()=>setIsNavOpened(!isNavOpened)} className="font-semibold opacity-80 hover:opacity-100">
